...
class Decoder_base(nn.Module):
    def __init__(self, z_latent_size, z_category_size, z_alpha_size, output_font_size=256*256):
        ...
        self.dfc4 = nn.Linear(2048, 4096)
        self.dfc5 = nn.Linear(4096, 8192)
        self.dfc6 = nn.Linear(font_size)
        
    def forward(self, z_latent, cat_vector, alpha_vector):
        z_latent = z_latent.view(z_latent.shape[0], -1)
        cat_vector = cat_vector.view(cat_vector.shape[0], -1)
        alpha_vector = alpha_vector.view(alpha_vector.shape[0], -1)
        
        z = torch.cat([z_latent, cat_vector, alpha_vector], dim=1)
